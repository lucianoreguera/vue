<template>
  <h2>Recorrido Array Simple</h2>
  <ul>
    <li v-for="country, index in countries" v-bind:key="index">{{ country }}</li>
  </ul>

  <h2>Recorrido Array de Objetos</h2>
  <ul>
    <li v-for="country in countries2" :key="country.id">{{ country.name }}</li>
  </ul>
</template>
...
<script lang="ts" setup>
  ...
  const countries:Ref<Array<string>> = ref([
    'Argentina', 'Brasil', 'Uruguay', 'México', 'Colombia', 'Perú'
  ])

  const countries2:Ref<Array<ICountry>> = ref([
    {
    id: 144,
    name: "Afganistán"
    },
